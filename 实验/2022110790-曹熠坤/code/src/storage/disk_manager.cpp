...
DiskManager::DiskManager() { memset(fd2pageno_, 0, MAX_FD * (sizeof(std::atomic<page_id_t>) / sizeof(char))); }//fd2pageno_清零

/**
 * @description: 将数据写入文件的指定磁盘页面中
 * @param {int} fd 磁盘文件的文件句柄
 * @param {page_id_t} page_no 写入目标页面的page_id（磁盘）
...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    off_t offset_in_file = page_no * PAGE_SIZE; // 计算页面在文件中的偏移量
    if(lseek(fd, offset_in_file, SEEK_SET) == -1) // 将文件描述符定位到文件头，指定写入的位置
    {
...
page_id_t DiskManager::allocate_page(int fd) {
    ...
    return fd2pageno_[fd]++;//文件所对应的（磁盘）页数加一
}
...
void DiskManager::create_file(const std::string &path) {
    ...
    // 注意不能重复创建相同文件
    if(is_file(path))//文件已经存在
    {
        throw FileExistsError(path);
    }
    
    int fd = open(path.c_str(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);   
    if(fd == -1)
    {
        throw UnixError();
    }
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    // 调用unlink()函数
    // 注意不能删除未关闭的文件
    if(!is_file(path)) //表明该文件不存在
    {
        throw FileNotFoundError(path);
    }

    if(path2fd_.count(path)) //表明已经打开了该文件
    {
        throw FileNotClosedError(path);
    }

    if(unlink(path.c_str()) == -1) // 删除指定路径的文件
    {
        throw UnixError();
    }
}
...
int DiskManager::open_file(const std::string &path) {
    ...
    if(path2fd_.count(path)) //表明已经打开了该文件
    {
        throw InternalError("DiskManager::open_file Error: File already opened");
    }

    int fd = open(path.c_str(), O_RDWR); // 以读写模式打开文件
    if(fd == -1) // 打开文件失败
    {
        throw UnixError();
    }
    ...
    fd2pageno_[fd] = 0; // 初始化文件的页面数量为0
    return fd; // 返回打开的文件的文件句柄
}
...
void DiskManager::close_file(int fd) {
    ...
    if(!fd2path_.count(fd)) //表明没有打开该文件
    {
        throw FileNotOpenError(fd);
    }
    if(close(fd) == -1) // 关闭文件描述符失败
    {
        throw UnixError();
    }

    std::string path = fd2path_[fd]; // 获取文件路径
    path2fd_.erase(path); // 从文件打开列表中删除该文件
    fd2path_.erase(fd); // 从文件打开列表中删除该文件
}
