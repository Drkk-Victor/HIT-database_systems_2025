...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    // Todo:
    // 1. 获取指定记录所在的page handle (rid.page_no -> page_handle)
    // 2. 初始化一个指向RmRecord的指针（赋值其内部的data和size）
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    if (page_handle.page_hdr->num_records > 0) {
        char* data = page_handle.get_slot(rid.slot_no); // get the slot address (rid.slot_no -> addr of slot)
        int size = file_hdr_.record_size;               // get the record size
        auto record = std::make_unique<RmRecord>(size, data); // create a new RmRecord
        return record;
    }
    return nullptr;
}
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if(buf == nullptr)  
    {
        throw InternalError("buf is null");
    }
    RmPageHandle page_handle = create_page_handle(); // 找到有空闲槽的页面
    ...
    if (page_handle.page_hdr->num_records == file_hdr_.num_records_per_page) {
        // page is full, update file header
        file_hdr_.first_free_page_no = page_handle.page_hdr->next_free_page_no;
        page_handle.page_hdr->next_free_page_no = RM_NO_PAGE; // 从空闲链表中把这页摘除
        // update file header in disk
    ...
    return Rid{page_handle.page->get_page_id().page_no, slot_no}; // return the rid of the inserted record
}
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    if(page == nullptr) {
        throw InternalError("page is null");
    }
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    ...
    if(page == nullptr) {
        throw InternalError("new page is null");
    }
    //初始化页头
