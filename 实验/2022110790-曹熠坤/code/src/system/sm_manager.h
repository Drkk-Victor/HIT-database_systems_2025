...
class SmManager {
   public:
    ...
    std::unordered_map<std::string, std::unique_ptr<RmFileHandle>> fhs_;    // table name -> record file handle, 当前数据库中每张表的数据文件
    std::unordered_map<std::string, std::unique_ptr<IxIndexHandle>> ihs_;   // index name -> index file handle, 当前数据库中每个索引的文件
   private:
